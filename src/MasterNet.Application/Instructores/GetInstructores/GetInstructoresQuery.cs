...
public class GetInstructoresQuery 
{
    ...
    : IRequestHandler<GetInstructoresQueryRequest, Result<PagedList<InstructorResponse>>>
    {
        ...
        )
        {
            ...
            {
                predicate = predicate
                .And(x=>x.Nombre!.Contains(request.InstructorRequest!.Nombre));
            }
             if(!string.IsNullOrEmpty(request.InstructorRequest!.Apellido))
            {
                predicate = predicate
                .And(x=>x.Apellidos!.Contains(request.InstructorRequest!.Apellido));
            }
              if(!string.IsNullOrEmpty(request.InstructorRequest!.OrderBy))
            {
                ...
                request.InstructorRequest.OrderBy!.ToLower() switch 
                {
                    ...
                    "apellidos"=> curso => curso.Apellidos!,
                    _=>curso=>curso.Nombre!
                };
                bool orderBy = request.InstructorRequest.OrderByAsc.HasValue
                ?
                request.InstructorRequest.OrderByAsc.Value
                :
                true;
                queryable = orderBy 
